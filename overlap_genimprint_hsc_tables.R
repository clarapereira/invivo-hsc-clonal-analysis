...
# # =======================================
# message(
#   "Total nonrandomly biased genes, B cells: ",
#   aici_table_long.expressed.lohwes.lohrna.biased.biascutof %>% 
#     select(ID, imprinted_status, LOH_ANYsamples, LOH_E6_RNA, imprinted_Bcells, imprinted_Tcells) %>% 
#     distinct() %>% 
#     filter(imprinted_Bcells == "genetically_biased") %>% nrow()
#     
# )
# 
# # B cells
# message(
#   "Total genes removed from analysis (including genetically_biased B cells, LOH_ANYsamples & LOH_E6_RNA genes: ",
#   aB <- aici_table_long.expressed.lohwes.lohrna.biased.biascutof %>% 
#     filter(cell_type == "Bcells") %>% 
#     select(ID, gene, imprinted_status, LOH_ANYsamples, LOH_E6_RNA, imprinted_Bcells, imprinted_Tcells) %>% 
#     distinct() %>% 
#     filter(
#       (imprinted_Bcells == "genetically_biased" | LOH_ANYsamples == "TRUE")
#       | LOH_E6_RNA == "TRUE"
#       ) %>% #group_by(imprinted_status) %>% summarise(n())
#     select(ID) %>% 
#     distinct() %>% 
#     nrow(),
#   "\nExcluding LOH_E6_RNA genes: ",
#   bB <- aici_table_long.expressed.lohwes.lohrna.biased.biascutof %>% 
#     filter(cell_type == "Bcells") %>% 
#     select(ID, gene, imprinted_status, LOH_ANYsamples, LOH_E6_RNA, imprinted_Bcells, imprinted_Tcells) %>% 
#     distinct() %>% 
#     filter(
...
# 
# # all cells: 
# df_aB <- aici_table_long.expressed.lohwes.lohrna.biased.biascutof %>% 
#   filter(cell_type == "Bcells") %>% 
#   select(ID, gene, imprinted_status, LOH_ANYsamples, LOH_E6_RNA, imprinted_Bcells, imprinted_Tcells) %>% 
#   distinct() %>% 
#   filter(
...
#     | LOH_E6_RNA == "TRUE"
#   )
# df_aT <- aici_table_long.expressed.lohwes.lohrna.biased.biascutof %>% 
#   filter(cell_type == "Tcells") %>% 
#   select(ID, gene, imprinted_status, LOH_ANYsamples, LOH_E6_RNA, imprinted_Bcells, imprinted_Tcells) %>% 
#   distinct() %>% 
#   filter(
#     (imprinted_Tcells == "genetically_biased" | LOH_ANYsamples == "TRUE")
#     #| LOH_E6_RNA == "TRUE"
#   ) 
# 
# genes_removed_total <- bind_rows(df_aB, df_aT) %>% 
#   select(ID, gene) %>% 
#   distinct() 
# 
# 
# message(
#   "Total genes removed from analysis (including genetically_biased, LOH_ANYsamples & LOH_E6_RNA genes: ",
#   aB <- aici_table_long.expressed.lohwes.lohrna.biased.biascutof %>% 
#     select(ID, gene, imprinted_status, LOH_ANYsamples, LOH_E6_RNA, imprinted_Bcells, imprinted_Tcells) %>% 
#     distinct() %>% 
#     filter(
#       (imprinted_Tcells == "genetically_biased" | imprinted_Bcells == "genetically_biased" | LOH_ANYsamples == "TRUE")
#       | LOH_E6_RNA == "TRUE" 
#     ) %>% #group_by(imprinted_status) %>% summarise(n())
#     select(ID) %>% 
#     distinct() %>% 
#     nrow(),
#   "\nExcluding LOH_E6_RNA genes: ",
#   b <- aici_table_long.expressed.lohwes.lohrna.biased.biascutof %>% 
#     select(ID, gene, imprinted_status, LOH_ANYsamples, LOH_E6_RNA, imprinted_Bcells, imprinted_Tcells) %>% 
#     distinct() %>% 
#     filter(
#       (imprinted_Tcells == "genetically_biased"  | imprinted_Bcells == "genetically_biased" | LOH_ANYsamples == "TRUE")
...
# )
# 
# # list genes removed total: 
# genes_removed_total <- aici_table_long.expressed.lohwes.lohrna.biased.biascutof %>% 
#   select(ID, gene, imprinted_status, LOH_ANYsamples, LOH_E6_RNA, imprinted_Bcells, imprinted_Tcells) %>% 
#   distinct() %>% 
#   filter(
#     (imprinted_Tcells == "genetically_biased" | imprinted_Bcells == "genetically_biased" | LOH_ANYsamples == "TRUE")
...
#   distinct()
# 
# # list genes removed no E6: 
# genes_removed_noE6 <- aici_table_long.expressed.lohwes.lohrna.biased.biascutof %>% 
#   select(ID, gene, imprinted_status, LOH_ANYsamples, LOH_E6_RNA, imprinted_Bcells, imprinted_Tcells) %>% 
#   distinct() %>% 
#   filter(
#     (imprinted_Tcells == "genetically_biased" | imprinted_Bcells == "genetically_biased" | LOH_ANYsamples == "TRUE")
...
# # 
# message(
#   "List of genes that would have not been removed if it weren't for E6 presumed LOH:"
# )
# setdiff(genes_removed_total, genes_removed_noE6) 
# # =======================================
